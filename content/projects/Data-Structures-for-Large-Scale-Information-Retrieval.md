...
categories: []
draft: false
description: "Inverted indexes are widely used by most large data retrieval systems because it supports keyword search in constant time. Compression codes can be applied to inverted indexes to reduce memory storage. This article gives an overview of various compression codes, with more details given on the optimizations and advantages of using Elias-Fano and Partitioned Elias-Fano compression codes. Finally, this article touches on the optimizations and use-cases for BitFunnel, an unusual probabilistic data structure used by the Bing search engine as an alternative to inverted indexes. BitFunnel has a compact representation and provides fast querying performance while avoiding costly global document ingestion operations."
---

5-minute high-level video:
